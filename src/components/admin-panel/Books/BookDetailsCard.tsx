...
function BookDetailsCard({ book }: { book: BookResponse }) {
  return (
    <div
      ...
      <div className="flex flex-col gap-2 p-4 text-xs relative">
        <div className="flex md:grid flex-col grid-cols-2 gap-y-1 md:gap-y-3">
          <span className="flex items-center">
            <div className="flex gap-2">
              ...
              <span className={` text-nowrap`}>{book?.title_fa}</span>
            </div>
          </span>
          <span className="flex items-center">
            <div className="flex gap-2">
              ...
              <span className={` text-nowrap`}>{book?.title_en}</span>
            </div>
          </span>
          <span className="flex items-center">
            ...
            <div className="flex gap-2">
              ...
              <span className={` text-nowrap`}>
                {toPersianDate(book?.created_at)}
                {' '}
                -
                {' '}
                {toPersianTime(book?.created_at)}
              </span>
            </div>
          </span>
        </div>
        ...
        <div className="flex flex-col md:flex-row col-span-1 gap-2 md:gap-4">
          <span className="flex items-center">
            <div className="flex gap-2 items-center">
              ...
              <span className="break-all pl-12.5">{book?.description}</span>
            </div>
          </span>
        </div>
        <div
        ...
        >
          <div className="flex flex-col scale-[0.85] gap-2">
            {/* <InsertBookDialog subcategory={book} button_size="icon" /> */}
            <UpdateCategoryDialog category={book} />
            <DeleteCategoryDialog category={book} />
          </div>
        </div>
      </div>
          ...
          {book?.poster_image
            ? (
                <div className="p-2 rounded-lg bg-black text-white">
                  <Image src={book?.poster_image} alt="" width={100} height={100} />
                </div>
              )
            : ' بدون تصویر'}
        </span>
