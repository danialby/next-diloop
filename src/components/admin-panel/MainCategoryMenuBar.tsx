    ...
    return (
        <div className={`relative flex justify-around font-vazir p-0 h-8 gap-0 rounded-full group z-3`}>
            ...
            <div className={`flex items-center justify-center my-0 w-1/2 rounded-none text-center cursor-pointer`}
                ...
                <span
                    className={`dark:text-black ${selectedMainCategory?.id === 2 && 'transition-color text-lg duration-300 font-black text-white dark:text-white '}`}>شاغل</span>
            </div>
            <div className={`flex items-center justify-center my-0 w-1/2 rounded-none text-center cursor-pointer`}
                 onClick={() => handleSelectMainCategory({id: 1, title: 'بیکار'})}>
                <span
                    className={`dark:text-black ${selectedMainCategory?.id === 1 && 'transition-color text-lg duration-300 font-black text-white dark:text-white '}`}>بیکار</span>
            </div>
        </div>
    )
}
